...
def send_media_safely(chat_id, media_url, media_type, caption_text):
    """
    Attempts to send media using direct URL first.
    If direct URL sending fails with content error (400), downloads to Koyeb local disk
    and uploads the local file cleanly, using 429 rate-limit proof wrappers!
    """
    try:
        # Try sending direct URL first (with rate limit support!)
        return send_telegram_media_with_rate_limit(chat_id, media_url, media_type, caption_text)
    except Exception as e:
        print("Direct URL sending failed (or CDN blocked), downloading to local disk as fallback:", str(e))

    # Fallback: Local download and upload
    temp_filename = f"/tmp/{uuid.uuid4()}.{'mp4' if media_type == 'video' else 'jpg'}"
    try:
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)'}
        response = requests.get(media_url, headers=headers, timeout=25)
        if response.status_code == 200:
            with open(temp_filename, "wb") as f:
                f.write(response.content)
            
            with open(temp_filename, "rb") as f:
                # Upload local file with complete 429 rate-limit proof wrapper!
            ...
            raise Exception(f"Failed to download media file locally. HTTP Status: {response.status_code}")
    except Exception as err:
        print("Local file fallback sending also failed:", str(err))
        try:
