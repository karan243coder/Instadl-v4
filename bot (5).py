...
def handle_profile_callbacks(call):
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            caption_parts = ["✨ <b>InstaMedia</b> ✨\n"]
            has_any_meta = False
            
            if action == "profile_dp":
                caption_parts.append(f"👤 Author: <b>@{username}</b>")
                caption_parts.append(f"💾 Type: <b>HD Profile Picture (DP)</b>")
            ...
            sent_msg = send_media_safely(chat_id, media_url, media_type, caption_text)

            # DB download Logging
            log_download_to_db(user_id, shortcode, media_type)

            # Forward logs in isolated thread to avoid delays!
            if LOG_CHANNEL_ID:
                log_thread = threading.Thread(
                    target=send_log_channel_forward,
                    args=(media_url, media_type, caption_text, call.from_user),
                    daemon=True
                )
                log_thread.start()
...
# -------------------------------------------------------------
def send_telegram_media_with_rate_limit(chat_id, media_data, media_type, caption_text):
    """
    Sends media to Telegram with automatic 429 Rate-Limit sleeping and retry loops.
    For photos, it sends BOTH a Photo (for preview) and a Document (for original quality).
    """
    retries = 5
    while retries > 0:
        try:
            if media_type == "video":
                return bot.send_video(chat_id, media_data, caption=caption_text, parse_mode="HTML")
            else:
                # 🖼️ 1. Send as Photo for quick preview in chat
                bot.send_photo(chat_id, media_data, caption=caption_text, parse_mode="HTML")
                
                # 🛠️ IMPORTANT: Reset file pointer to the beginning so we can read it again for the document
                if hasattr(media_data, 'seek'):
                    media_data.seek(0)
                
                # 💎 2. Send as Document for 100% original quality and zero cropping
                doc_caption = f"{caption_text}\n\n💎 <b>Original Quality (No Crop)</b>"
                return bot.send_document(chat_id, media_data, caption=doc_caption, parse_mode="HTML")
                
        except ApiTelegramException as e:
            if e.error_code == 429:
                retry_after = getattr(e, 'retry_after', None)
                if not retry_after:
                    match = re.search(r"retry after (\d+)", e.description)
                    retry_after = int(match.group(1)) if match else 5
                print(f"😴 Rate limited! Telegram asked to wait for {retry_after}s. Sleeping...")
            ...
            if "Too Many Requests" in str(e) or "429" in str(e):
                ...
                retry_after = int(match.group(1)) if match else 5
                print(f"😴 Rate limited (fallback)! Sleeping for {retry_after}s...")
                time.sleep(retry_after + 1)
                retries -= 1
            else:
                raise e
    raise Exception("Telegram rate-limit threshold exceeded. Failed to send media after multiple retries.")

def send_media_safely(chat_id, media_url, media_type, caption_text):
    """
    🚀 PRO-LEVEL MEDIA DELIVERY:
    Forces local download for ALL photos to bypass Telegram's URL-based compression 
    and maintain original aspect ratios (Portrait/Landscape/9:16).
    """
    temp_filename = f"/tmp/{uuid.uuid4()}.{'mp4' if media_type == 'video' else 'jpg'}"
    
    try:
        # 🛡️ High-End User-Agent to ensure we get the full-resolution image from CDN
        headers = {
        ...
        # Download the file to local buffer
        response = requests.get(media_url, headers=headers, timeout=25, stream=True)
        ...
        if response.status_code == 200:
            ...
            with open(temp_filename, "rb") as f:
                # Upload local file to preserve quality and aspect ratio
                sent = send_telegram_media_with_rate_limit(chat_id, f, media_type, caption_text)
            ...
            # If local download fails, try the direct URL as a last resort
            print(f"Local download failed (Status: {response.status_code}), trying direct URL...")
            return send_telegram_media_with_rate_limit(chat_id, media_url, media_type, caption_text)

    except Exception as err:
        print("Pro Media Delivery failed, falling back to direct URL:", str(err))
        try:
            return send_telegram_media_with_rate_limit(chat_id, media_url, media_type, caption_text)
        except Exception as final_err:
            print("All delivery methods failed:", str(final_err))
            raise final_err
...
def handle_instagram_link(message):
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            # Log Channel Forwarding in background thread!
            if LOG_CHANNEL_ID:
                log_thread = threading.Thread(
                    target=send_log_channel_forward,
                    args=(media_url, media_type, caption_text, message.from_user),
                    daemon=True
                )
                log_thread.start()
