...
def send_album_safely(chat_id, items, caption_text):
    """Sends media as a Preview Album and a Document Album for high quality."""
    downloaded_files = []
    for item in items:
    ...
    try:
        ...
        for i, file in enumerate(downloaded_files):
            with open(file["path"], 'rb') as f:
                if file["type"] == "video": preview_group.append(telebot.types.InputMediaVideo(f, caption=caption_text if i == 0 else None))
                else: preview_group.append(telebot.types.InputMediaPhoto(f, caption=caption_text if i == 0 else None))
        for i in range(0, len(preview_group), 10): bot.send_media_group(chat_id, preview_group[i:i+10])
        document_group = []
        for i, file in enumerate(downloaded_files):
            with open(file["path"], 'rb') as f:
        ...
        for i in range(0, len(document_group), 10): bot.send_media_group(chat_id, document_group[i:i+10])
    except Exception as e:
...
def send_album_safely(chat_id, items, caption_text):
    """
    🚀 PRO ALBUM DELIVERY:
    Sends all media items as two separate groups: 
    1. A Preview Album (Photos/Videos)
    2. A High-Quality Album (Documents)
    """
    downloaded_files = []
    
    # 1. Download all items first
    for item in items:
        path = download_media_locally(item["url"], item["type"])
        if path:
            downloaded_files.append({"path": path, "type": item["type"]})

    if not downloaded_files:
        return False

    try:
        # 2. Prepare Preview Group
        preview_group = []
        for i, file in enumerate(downloaded_files):
            with open(file["path"], 'rb') as f:
                if file["type"] == "video":
