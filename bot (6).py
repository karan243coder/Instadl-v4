...
def send_telegram_media_with_rate_limit(chat_id, media_data, media_type, caption_text):
    ...
    Photos are sent as DOCUMENTS to ensure maximum possible quality provided by the API.
    """
    retries = 5
    while retries > 0:
        try:
            if media_type == "video":
                ...
                # 💎 PRO DELIVERY: Sending as Document is the only way to avoid 
                # Telegram's internal cropping and compression.
                return bot.send_document(chat_id, media_data, caption=caption_text, parse_mode="HTML")
