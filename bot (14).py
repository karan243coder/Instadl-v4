...
def send_log_channel_forward(items, caption_text, user_obj):
    """
    🚀 ROBUST LOG FORWARDING:
    Sends the entire album to the log channel using the same robust delivery system.
    """
    ...
    try:
        ...
        send_album_safely(LOG_CHANNEL_ID, items, log_caption)
    except Exception as log_err:
        print("Log channel album forwarding failed:", str(log_err))
                ...
                if new_items:
                    ...
                    for item in new_items:
                        ...
                        try:
                            ...
                            send_album_safely(admin_id, [{"url": media_url, "type": media_type}], caption_text)

                            # 2. Deliver directly to Log Channel if configured!
                            if LOG_CHANNEL_ID:
                                try:
                                    log_caption = f"🔄 <b>[AUTO-DOWNLOAD SYSTEM LOG]</b>\n\n{caption_text}"
                                    send_album_safely(LOG_CHANNEL_ID, [{"url": media_url, "type": media_type}], log_caption)
                                except Exception as log_err:
...
def handle_profile_callbacks(call):
    ...
    try:
        ...
        if LOG_CHANNEL_ID:
            threading.Thread(target=send_log_channel_forward, args=(items, album_caption, call.from_user), daemon=True).start()
...
def handle_instagram_link(message):
    ...
    try:
        ...
        if LOG_CHANNEL_ID:
            threading.Thread(target=send_log_channel_forward, args=(items, album_caption, message.from_user), daemon=True).start()
